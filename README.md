...
- 이력서 접근 제한은 실제 로그인이 아니라 프론트엔드 세션 스코프(sessionStorage)로만 구현되어 있어, 의도적으로 우회하려는 접근까지는 막지 못합니다.
- `member_id`는 현재 `1`로 고정되어 있어 사실상 단일 사용자 앱입니다.
- "자소서 초안 생성" 버튼은 화면에는 있지만 해당 기능(백엔드 엔드포인트)은 아직 구현되어 있지 않습니다.
